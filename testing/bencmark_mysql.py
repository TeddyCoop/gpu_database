...
def benchmark_mysql_query(csv_path, sql_query, host, user, password, database, in_memory, use_index):
    ...
    col_defs = [f"`{col}` VARCHAR(255)" for col in df.columns]
    col_defs[0] = f"`{index_col}` VARCHAR(255){' PRIMARY KEY' if use_index else ''}"
    engine = "ENGINE=MEMORY" if in_memory else "ENGINE=InnoDB"
    create_stmt = f'CREATE TABLE `{table_name}` ({", ".join(col_defs)}) {engine};'
...
if __name__ == "__main__":
    ...
    benchmark_mysql_query(args.csv, args.query, args.host, args.user, args.password, args.database, args.in_memory, args.use_index)
